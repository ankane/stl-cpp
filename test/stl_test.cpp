...
void test_stl_works() {
    ...
    assert_elements_in_delta(
        {0.36926576, 0.75655484, -1.3324139, 1.9553658, -0.6044802}, first(fit.seasonal(), 5)
    );
    assert_elements_in_delta(
        {4.804099, 4.9097075, 5.015316, 5.16045, 5.305584}, first(fit.trend(), 5)
    );
    assert_elements_in_delta(
        {-0.17336464, 3.3337379, -1.6829021, 1.8841844, -4.7011037}, first(fit.remainder(), 5)
    );
...
void test_stl_robust() {
    ...
    assert_elements_in_delta(
        {0.14922355, 0.47939026, -1.833231, 1.7411387, 0.8200711}, first(fit.seasonal(), 5)
    );
    assert_elements_in_delta(
        {5.397365, 5.4745436, 5.5517216, 5.6499176, 5.748114}, first(fit.trend(), 5)
    );
    assert_elements_in_delta(
        {-0.5465884, 3.0460663, -1.7184906, 1.6089439, -6.5681853}, first(fit.remainder(), 5)
    );
    assert_elements_in_delta(
        {0.99374926, 0.8129377, 0.9385952, 0.9458036, 0.29742217}, first(fit.weights(), 5)
    );
}
...
void test_stl_too_few_periods() {
    assert_exception<std::invalid_argument>(
        []() { Stl<T>{generate_series<T>(), 16}; }, "series has less than two periods"
    );
}
